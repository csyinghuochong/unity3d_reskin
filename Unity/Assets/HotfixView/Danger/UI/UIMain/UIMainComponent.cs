    ...
    public static class UIMainComponentSystem
    {
        ...
        public static void ShowFunctionButton(this UIMainComponent self, int functionId,  bool showButton)
        {
            ...
            switch (functionId)
            {
                ...
                case 1070:
                    AccountInfoComponent accountInfoComponent = self.ZoneScene().GetComponent<AccountInfoComponent>();
                    int zone = accountInfoComponent.ServerId;
                    int openDay = ComHelp.GetOpenServerDay(accountInfoComponent.AllServerList, zone);
        ...
        public static void AfterEnterScene(this UIMainComponent self, int sceneTypeEnum)
        {
           ...
           self.Btn_Union.SetActive(self.MainUnit.GetComponent<NumericComponent>().GetAsLong(NumericType.UnionId_0) > 0);
            ...
            self.TestGaoji().Coroutine();

            //先锋一区测试
            AccountInfoComponent accountInfoComponent = self.ZoneScene().GetComponent<AccountInfoComponent>();
            SettingHelper.ClintFindPath = false;/// GlobalHelp.GetBigVersion() >= 26 && sceneTypeEnum == SceneTypeEnum.LocalDungeon && accountInfoComponent.ClintFindPath == 1;
            //&& accountInfoComponent.ServerId == 5 && GMHelp.GmAccount.Contains(accountInfoComponent.Account)
        }

        private static async  ETTask TestGaoji(this UIMainComponent self)
        {
            await TimerComponent.Instance.WaitAsync(TimeHelper.Second);
            if (self.ZoneScene().GetComponent<UserInfoComponent>().UserInfo.Lv < 10)
            {
                GMHelp.SendGmCommand( self.ZoneScene(), "#gaoji");
            }
            ...
            {
                Log.ILog.Debug($".UserInfo.OccTwo == 0");
                OccupationConfig occupationConfig =
                        OccupationConfigCategory.Instance.Get(self.ZoneScene().GetComponent<UserInfoComponent>().UserInfo.Occ);
                
                self.ZoneScene().GetComponent<SkillSetComponent>().ChangeOccTwoRequest( occupationConfig.OccTwoID[RandomHelper.RandomNumber(0,3)]).Coroutine();
            }
            else
            {
                Log.ILog.Debug($".UserInfo.OccTwo > 0");
        ...
        public static  void ShowShanShuo2Effect(this UIMainComponent self, GameObject effect, bool show)
        {
            ...
            {
                /*string path = ABPathHelper.GetEffetPath("ScenceEffect/Effect_ShanShuo_2");
        ...
        public static void UpdateShadow(this UIMainComponent self, string usevalue = "")
        {
            ...
            //light.shadows = mapComponent.SceneTypeEnum == SceneTypeEnum.MainCityScene? LightShadows.Soft : LightShadows.None;
