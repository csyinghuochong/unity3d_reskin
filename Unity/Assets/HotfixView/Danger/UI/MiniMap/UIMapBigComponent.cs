...
namespace ET
{
    ...
    public static class UIMapBigComponentSystem
    {
        public static async ETTask OnAwake(this UIMapBigComponent self)
        {
            ...
            if (mapCamera == null)
            {
                var path = ABPathHelper.GetUnitPath("Common/MapCamera");
        ...
        public static async ETTask RequestLocalUnitPosition(this UIMapBigComponent self)
        {
            ...
            foreach (UnitInfo unitInfo in response.UnitList)
            {
                ...
                {
                    self.InstantiateIcon(self.bossIcon, vector3, monsterConfig.GetMonsterName());
                }
                ...
                {
                    self.InstantiateIcon(self.jinglingIcon, vector3, monsterConfig.GetMonsterName());
                }
            }
        }
        ...
        public static  void InitNpcList(this UIMapBigComponent self)
        {
            ...
            if (SceneConfigHelper.UseSceneConfig(mapComponent.SceneTypeEnum))
            {
                ...
                self.MapName.GetComponent<Text>().text = SceneConfigCategory.Instance.Get(self.SceneId).GetName();
                self.ShowStallArea();
            }
            if (mapComponent.SceneTypeEnum == (int)SceneTypeEnum.LocalDungeon)
            {
                npcList = DungeonConfigCategory.Instance.Get(self.SceneId).NpcList;
                self.MapName.GetComponent<Text>().text = DungeonConfigCategory.Instance.Get(self.SceneId).GetChapterName();
                self.ShowChuansong();
            ...
            if (npcList != null)
            {
                for (int i = 0; i < npcList.Length; i++)
                {
                    ...
                    self.InstantiateIcon(self.NpcPostion, new Vector3(npcConfig.Position[0] * 0.01f, npcConfig.Position[2] * 0.01f, 0), npcConfig.GetName());
        ...
        public static void OnClickNpcItem(this UIMapBigComponent self,  int unitype, int configid)
        {
            ...
            {
                unit.MoveToAsync2(self.BossList[configid], true, null, 0, 0, true).Coroutine();
            }
        }
