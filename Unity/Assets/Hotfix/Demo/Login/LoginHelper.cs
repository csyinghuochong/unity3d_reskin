...
namespace ET
{
    public static class LoginHelper
    {
        ...
        public static async ETTask<int> Login(Scene zoneScene, string address, string account, string password, bool relink, string token, string thirdLogin, string devivename)
        {
            ...
            playerComponent.NewRegister = a2CLoginAccount.NewRegister;
        ...
        public static async ETTask<int> EnterGame(Scene zoneScene, string devicename, bool relink , int plaform)
        {
            ...
            {
                Log.Error($"g2CEnterGame.Error.:{g2CEnterGame.Error} {accountInfoComponent.Account}");
        ...
        public static async ETTask<int> DeleteAccount(Scene zoneScene, bool outNet, VersionMode versionCode, string account, string password, int platform, int serverid)
        {
            try
            {
                ...
                IPAddress[] xxc = Dns.GetHostEntry(ComHelp.GetLogicServer(!outNet)).AddressList;
                //走的中心服
        ...
        public static async ETTask<int> Register(Scene zoneScene, bool outNet,  VersionMode versionCode, string account, string password, int platform, int serverid)
        {
            try
            {
                ...
                address = outNet ? $"{ComHelp.LogicServerBanHao}:{GetAccountCenterPort(versionCode)}" : $"{ComHelp.LocalIp}:{GetAccountCenterPort(versionCode)}";
        ...
        public static async ETTask<int> SendSmsVerifyCode(Scene zoneScene, bool innerNet, VersionMode versionMode,  string phoneNum, int platform, int serverid)
        {
            try
            {
                ...
                {
                    IPAddress[] xxc = Dns.GetHostEntry(ComHelp.GetLogicServer(innerNet)).AddressList;
        ...
        public static async ETTask<int> OnServerListAsyncRelease(Scene zoneScene,VersionMode versionMode, string account, int platform, int serverid)
        {
            try
            {
                ...
                //IPAddress[] xxc = Dns.GetHostEntry(ComHelp.GetLogicServer(false)).AddressList;
                string address = $"{ComHelp.GetLogicServer(false)}:{GetAccountCenterPort(versionMode)}";
                AccountInfoComponent accountInfoComponent = zoneScene.GetComponent<AccountInfoComponent>();
                Session session = zoneScene.GetComponent<NetKcpComponent>().Create(NetworkHelper.ToIPEndPoint(address));
                {
