    ...
    public static class RobotCaseSystem
    {
        ...
        public static async ETTask<Scene> NewRobot(this RobotCase self, int zone, string name)
        {
            ...
            try
            {
                ...
                await LoginHelper.Login(zoneScene, ComHelp.GetGetServerItem(accountInfoComponent.AllServerList, 1).ServerIp, zone.ToString(), zone.ToString(), false, string.Empty, "0", string.Empty  );
                ...
                await LoginHelper.EnterGame(zoneScene, "", false, 0);
